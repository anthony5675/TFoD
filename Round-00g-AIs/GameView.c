...
struct gameView {
    ...
    //Stores the messages from the previous round.
    PlayerMessage lastMessages[NUM_PLAYERS];
};
    ...
    if (view->round != 0) {
        ...
        for (l = 0; l < NUM_PLAYERS; l++) {
            for (m = 0; m < MESSAGE_SIZE; m++) {
                view->lastMessages[l][m] = messages[(view->round - 1) * 5 + l][m];
            }   
        }
    }
...
void getMessage (GameView currentView, PlayerID player, PlayerMessage array) {

    int i = 0;
    if (player < 0 || player > 4) {
        array[0] = '\0';
    } else {
        while (currentView->lastMessages[player][i] != '\0') {
            array[i] = currentView->lastMessages[player][i];
            i++;
        }
        array[i] = '\0';
    }
