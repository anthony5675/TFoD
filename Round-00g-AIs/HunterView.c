...
void giveMessage (HunterView currentView, PlayerID player, PlayerMessage array) {

    getMessage(currentView->game, player, array);
}

//Returns the abbrev of the current player's location.
char *myLocation (HunterView currentView) {
    LocationID temp = 0;
    temp = getLocation(currentView->game, getCurrentPlayer(currentView->game));
    return (idToAbbrev (temp));
}
